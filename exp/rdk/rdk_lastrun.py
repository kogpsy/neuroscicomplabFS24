...
# Run 'Before Experiment' code from codeParticipantID
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/dafitze/UNI/GIT/neuroscicomplabFS24/exp/rdk/rdk_lastrun.py',
...
def run(expInfo, thisExp, win, inputs, globalClock=None, thisSession=None):
    ...
    textStaircaseInstr = visual.TextStim(win=win, name='textStaircaseInstr',
        ...
        languageStyle='LTR',
        depth=0.0);
    # Run 'Begin Experiment' code from codeStaircaseInstr
    win.color = 'black'
    ...
    # Unknown component ignored: goodbye_2
    ...
    thisExp.addData('participantID.started', globalClock.getTime())
    ...
    thisExp.addData('participantID.stopped', globalClock.getTime())
    ...
    for thisPractice_run_loop in practice_run_loop:
        ...
        stimulusDots.setFieldCoherence($level)
        ...
        while continueRoutine and routineTimer.getTime() < 1.5:
            ...
            if stimulusDots.status == STARTED:
                if frameN >= (stimulusDots.frameNStart + 190):
                    # keep track of stop time/frame for later
            ...
            # if respDots is stopping this frame...
            if respDots.status == STARTED:
                if frameN >= (respDots.frameNStart + 90):
                    ...
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'respDots.stopped')
                    # update status
    ...
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('staircase_instruction.started', globalClock.getTime())
    ...
    while continueRoutine:
        ...
        # if textStaircaseInstr is starting this frame...
        if textStaircaseInstr.status == NOT_STARTED and tThisFlip >= 0-frameTolerance:
            ...
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'textStaircaseInstr.started')
            # update status
            textStaircaseInstr.status = STARTED
            textStaircaseInstr.setAutoDraw(True)
        
        # if textStaircaseInstr is active this frame...
        if textStaircaseInstr.status == STARTED:
            # update params
            pass
        ...
        # if respStaircaseInstr is starting this frame...
        if respStaircaseInstr.status == NOT_STARTED and tThisFlip >= 3-frameTolerance:
            ...
            # add timestamp to datafile
            thisExp.timestampOnFlip(win, 'respStaircaseInstr.started')
            # update status
        ...
        if respStaircaseInstr.status == STARTED and not waitOnFlip:
            theseKeys = respStaircaseInstr.getKeys(keyList=['space'], ignoreKeys=["escape"], waitRelease=False)
            ...
            if len(_respStaircaseInstr_allKeys):
                ...
                respStaircaseInstr.duration = _respStaircaseInstr_allKeys[-1].duration
                # a response ends the routine
                continueRoutine = False
    ...
    for thisComponent in staircase_instructionComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('staircase_instruction.stopped', globalClock.getTime())
    # check responses
    ...
    if respStaircaseInstr.keys != None:  # we had a response
        ...
        thisExp.addData('respStaircaseInstr.duration', respStaircaseInstr.duration)
    thisExp.nextEntry()
    ...
    staircase_run_loop = data.StairHandler(startVal=0.5, extraInfo=expInfo,
        stepSizes=[0.1, 0.1, 0.08, 0.08, 0.08, 0.05], stepType='lin',
        nReversals=7.0, nTrials=20.0, 
        nUp=1.0, nDown=3.0,
    ...
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('speed_instruction.started', globalClock.getTime())
    respSpeedInstr.keys = []
    respSpeedInstr.rt = []
    ...
    for thisComponent in speed_instructionComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('speed_instruction.stopped', globalClock.getTime())
    # check responses
    ...
    if thisSpeed_run_loop != None:
        for paramName in thisSpeed_run_loop:
            globals()[paramName] = thisSpeed_run_loop[paramName]
    ...
    for thisSpeed_run_loop in speed_run_loop:
        ...
        if respDots.keys != None:  # we had a response
            ...
            speed_run_loop.addData('respDots.duration', respDots.duration)
        # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
        if routineForceEnded:
            routineTimer.reset()
        else:
    ...
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('accuracy_instruction.started', globalClock.getTime())
    ...
    for thisComponent in accuracy_instructionComponents:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('accuracy_instruction.stopped', globalClock.getTime())
    # check responses
    ...
    if thisAccuracy_run_loop != None:
        for paramName in thisAccuracy_run_loop:
            globals()[paramName] = thisAccuracy_run_loop[paramName]
    ...
    for thisAccuracy_run_loop in accuracy_run_loop:
        ...
        if respDots.keys != None:  # we had a response
            ...
            accuracy_run_loop.addData('respDots.duration', respDots.duration)
        # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
        if routineForceEnded:
            routineTimer.reset()
        else:
